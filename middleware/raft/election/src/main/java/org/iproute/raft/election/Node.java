        ...
        try {
            ...
            if (this.getRole() == Role.Candidate) {
                if (voteReq.getIterm() == this.getIterm()) {
                    log.info("当前节点{}是候选人,接收到另外一个候选人{}的投票 | 接收的数据 (投票人={},任期={}), 自己的数据(节点名称={},任期={}),任期相同，说明有了多个候选者，{}先成为候选者，{}后成为候选者并把自己的任期+1了", this.getName(), voteReq.getCandidateLeader(), voteReq.getCandidateLeader(), voteReq.getIterm(), this.getName(), this.getIterm(), this.getName(), voteReq.getCandidateLeader());
                    mockNetDelay();
                    return new VoteRsp(this.getIterm(), this.leaderName);
                } else if (voteReq.getIterm() > this.iterm) {
                    log.info("当前节点{}是候选人,接收到另外一个候选人{}的投票 | 接收的数据 (投票人={},任期={}), 自己的数据(节点名称={},任期={}),接收的任期 > 自己的任期 ,同意投票人{}当选Leader, 并且自己降级为Follower", this.getName(), voteReq.getCandidateLeader(), voteReq.getCandidateLeader(), voteReq.getIterm(), this.getName(), this.getIterm(), voteReq.getCandidateLeader());
                    // 主动降级！！！
                    this.setRole(Role.Follower);
                    // 修改任期 和 同意的 leader
